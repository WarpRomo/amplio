...
export interface RunDetail {
  ...
  /** Prompt sections resolved at creation (may be empty). */
  briefings: string[];
}
...
/** A configured model: what config.toml says, plus its short display label. */
...
export interface AboutInfo {
  ...
  config_path: string;
  logs_dir: string;
  default_llm: ModelRef;
  models: ModelRef[];
  system_llm_hq: ModelRef;
  system_llm_fast: ModelRef;
  owner: string;
  auth_on: boolean;
...
/** One entry of the briefing library (GET /api/briefings). Bodies are not served:
 *  the picker needs identity and description, not several KB of prompt per row. */
